...
class RestaurantController extends GetxController implements GetxService {
  ...
  Future<void> addProduct(Product product, String token, bool isAdd) async {
    _isLoading = true;
    update();
    ...
    if(_variantTypeList.length > 0) {
      ...
      _attributeList.forEach((attributeModel) {
        if(attributeModel.active) {
          ...
          attributeModel.variants.forEach((variant) {
            _variantString = _variantString + '${_variantString.isEmpty ? '' : ','}' + variant;
          });
      ...
      for(int index=0; index<_variantTypeList.length; index++) {
        _fields.addAll(<String, String> {'price_${_variantTypeList[index].variantType}': _variantTypeList[index].controller.text});
      }
    }
    Response response = await restaurantRepo.addProduct(product, _pickedLogo, _fields, token, isAdd);
    if(response.statusCode == 200) {
      Get.back();
  ...
  void initRestaurantData(Restaurant restaurant) {
    ...
    _weekendString = restaurant.offDay.trim();
